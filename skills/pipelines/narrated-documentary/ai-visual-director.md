...
- **Explanatory / count-critical beats use the FIRST-LAST-FRAME lane, NOT open i2v.** When a beat must teach a precise fact open i2v structurally drifts on — an exact object count, "exactly N of M change," a locked camera, a clean state transition — do not prompt Grok for it (it has no end-frame input, so it dollies, miscounts, and applies global washes; proven repeatedly on the seg_006 "six→three" beat). Instead author **two matched-framing keyframes** (identical camera/lens/palette and composition; only the target objects differ, **small delta**) and interpolate with **`vr.generate_flf_shot(start_url, end_url, prompt, duration_s, out)`** → Kling 3.0 on Kie (`tools/video/kling_kie_video.py`, model `kling-3.0/video`, `image_urls=[start,end]`, `multi_shots:false`, ~$0.084/s std). The precision lives in the hand-authored endpoints, not the model. **Derive the end frame deterministically from the start** where you can (e.g. drain three of six figures' light toward navy with a soft ramp) — a generative "edit" of the start drifts the composition and breaks FLF. One transition per clip; chain legs for multi-step arcs (six→three→zero). Keep Grok for atmospheric/emotional shots, where free motion is the strength.
