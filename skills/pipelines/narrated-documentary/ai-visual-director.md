...
- **Explanatory / count-critical beats use the FIRST-LAST-FRAME lane, NOT open i2v.** When a beat must teach a precise fact open i2v structurally drifts on — an exact object count, "exactly N of M change," a locked camera, a clean state transition, or **objects that must not appear/disappear** (the seg_009 morphing-equipment-room failure) — do not prompt Grok for it (it has no end-frame input, so it dollies, miscounts, applies global washes, and hallucinates props across chained legs). Instead use the FLF lane: **`lib.flf.flf_beat(start_png, prompt, window_s, out, derive=…)`** — it hosts the start, derives the end, interpolates via Kling 3.0 on Kie (`vr.generate_flf_shot` → `tools/video/kling_kie_video.py`, model `kling-3.0/video`, `image_urls=[start,end]`, `multi_shots:false`, ~$0.084/s std), and conforms to the slot. The precision lives in the hand-authored endpoints, not the model. Three rules that make it reliable:
